...
- **Spacing system** (use these, never ad-hoc pixel values): gaps come in exactly three sizes via tokens `--gap-s` (6px, micro), `--gap-m` (10px, standard: list rhythm, element gaps), `--gap-l` (12px, large clusters). Card interiors use `--pad-card` (14px 16px); sheet rows, inputs, and list rows use `--pad-row` (12px 14px). Stacked cards/rows separate with `margin-bottom:var(--gap-m)`; sections separate with 20-22px. Pills are 8px 13px, small chips 7px 12px, primary buttons 10px 14px. Radii come in exactly three tokens plus two shapes: `--r` (16px, cards and sheets), `--r-sm` (12px, rows, inputs, buttons, dots), `--r-xs` (8px, tiny badges and photo thumbs), `99px` for pills, `50%` for circles. Never introduce a raw radius value.
